...
export const WorkLeft : React.FC<{progress : number}>= ({children,  progress}) => {
    ...
    if (progress > .85 ) {
        translateY = Math.max(-50, -(progress - .85) * 2 * 50)
    }
    return (
...
export const WorkLine : React.FC<LinkProps> = ({href, children}) => (
    <Link href={href}>
        <a target='_blank' className='underline underline-offset-8 decoration-1' rel='noreferrer'>
            {children}
        </a>
    </Link>
)